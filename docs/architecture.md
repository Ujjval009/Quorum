...
flowchart LR
    ...
    backend -->|generate grounded answer| llm
    ...
    ingestion -->|create embeddings| llm
...
- Groq SDK (OpenAI-compatible) for generation and embeddings
...
headers: async () => ({
      ...
      Authorization: `Bearer ${awaitgetAccessToken()}`,
...
1. Embed the user's query with the configured embedding model (Groq OpenAI-compatible endpoint).
